...
func main() {
	...
	if lastDeployAddress == "" {
		var err error
		basicNft, err = script.DeployBasicNft(privateKey, client, 11155111)
		if err != nil {
	...
	if balance.Uint64() == 0 {
		_, err = script.MintNft(privateKey, client, basicNft, "ipfs://QmYJouWczWMFXeA4UwQVHrJM4oe9j7wdMcvrkFtEosiFVR?filename=jiangjiang.json", 11155111)
		if err != nil {
